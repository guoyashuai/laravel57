...
if (!function_exists('admin_base_path')) {
     ...
     * @return string
     */
    function admin_base_path($path = '', $parameters = [], $secure =null)
    {
        ...
        if (is_null($path) || strlen($path) == 0) {
            //return $prefix;
            return url(config('admin.route.prefix'));
        }

        //return $prefix.'/'.$path;
        return url(($prefix. '/'.$path), $parameters, $secure);
    }
}
...
if (!function_exists('admin_asset')) {
    ...
    {
        //地址如果是https就直接返回域名地址，没有就去应用实例地址
